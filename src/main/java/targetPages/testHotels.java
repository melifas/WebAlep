...
public class testHotels {
    public void print() {
        final String city = "Αθήνα";
        final String startdate = "2020-03-15";
        //TODO Να βρώ ενα τρόπο να αυξανω την ημερομηνια κατα 1
        final String enddate = increasDateByOne(startdate);
        RecordsDAO dao = new RecordsDAO();
        Document page;
        {
            try {
                page = Jsoup.connect("https://el.hotels.com/search.do?q-destination=" + URLEncoder.encode(city,"UTF-8")+"&q-check-in="+ URLEncoder.encode(startdate,"UTF-8")
                ...
                for (int i =0; i< hotelPricelink.size(); i++){
                    ...
                    if (priceString.length()>7){
                        String pricenotready = priceString.substring(3);
                ...
                if (hotelNames.size()!=0) {
                    ...
                    System.out.println("Μέσος όρος τιμών ξενοδοχείων απο hotel.gr για την αναζήτηση " + city + " την ημερομηνία "+ startdate+ " είναι "+ df.format(averagePrice)+"");
                }else{
